...
- Скриптовый язык (продолжение): **функции** (`name() { … }`, позиционные
  `$1..$9`/`$#` с восстановлением, `return N`, тела с любыми
  конструкциями), **case** (`*`-глоб, `|`-альтернативы, `;;`, одно- и
  многострочные ветви), **until**; off-by-one в закрытии `$(( ))`
  (съедал символ после `))` — пойман живым демо + host-тестом);
  shell 24 теста, workspace 138/0; демо /etc/demo.sh в каждом буте
  (CI-маркеры fn/case/until).
