...
## Скриптовый язык Orbita (контракт платформы)

**Каждая возможность ОС должна быть управляемой из скриптов и команд
shell'а.** Скриптовый язык — не надстройка, а контракт платформы:

- **Команды скриптов = команды shell'а** (встроенного в базовую ОС):
  Linux-совместимые (`ls`, `cat`, `echo`, `cd`, `pwd`, `env`, `grep`,
  `wc`, `head`, `tail`, `test`/`[`, `sh`, `source`, `pkg`, `run`,
  `ping`, `ps`, …) плюс расширенные Orbita (`netcfg`, `apps`,
  `services`, `launch`, `svcrun`, `desktop`, `events`).
- **Управление потоком** как в `sh`: `if/elif/else/fi`, `while/do/done`,
  `for VAR in …`, `break`, `continue`, `exit N`, цепочки `&&`/`||`,
  переменные `$VAR` с присваиванием, кавычки, пайпы `|`, редиректы
  `>`/`>>`/`<`, комментарии `#`.
- **Запуск файлов как в Linux**: `sh script.sh`, `./script.sh`,
  `/etc/demo.sh` — любой путь до существующего файла исполняется как
  скрипт.
- **Правило для модулей**: при разработке любого модуля проверяй, что
  его возможности манипулируемы командами и скриптами; если нет —
  добавь команду/подкоманду (см. `docs/scripting.md`, раздел
  «Скриптуемость модулей»).

**Полный справочник языка и всех команд**: [`docs/scripting.md`](docs/scripting.md).
Живое доказательство: `/etc/demo.sh` исполняется при каждом буте
(QEMU-маркеры `script: if ok`, `script: and-ok`).
